...
namespace BizMate.Application.Common.Interfaces.Repositories
{
    ...
    {
        Task<(List<ExportReceipt> Receipts, int TotalCount)> SearchReceiptsWithPaging(Guid storeId, DateTime? dateFrom, DateTime? dateTo, string? statusCode, string? keyword, int pageIndex, int pageSize, QueryFactory queryFactory);
